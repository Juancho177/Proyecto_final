...
# === NUEVO: API para Streamlit: generador frame a frame ======================
def run_stream(
    video_source: str | int,
    *,
    weights_path: str | None = None,
    lanes_config: str | None = None,
    headless: bool = True,
    save_path: str | None = None,
    ctrl_overrides: dict | None = None,
    app_overrides: dict | None = None,
):
    """
    Generador que emite (frame_bgr, stats) para cada iteración del pipeline.

    Params
    ------
    video_source: "0" para webcam, ruta a archivo local, o URL RTSP/RTMP.
    weights_path: ruta a pesos YOLO (opcional; si no, usa los de config).
    lanes_config: ruta a lanes.json (opcional; si no, usa config por defecto).
    headless: no abre ventanas (pensado para UI/Streamlit).
    save_path: si se define, guarda un MP4 mientras emite frames.
    ctrl_overrides, app_overrides: dicts para sobreescribir parámetros de
        control (min_green, clearance, etc.) y app (conf, iou, tracker_cfg...).

    Yields
    ------
    frame_bgr: numpy.ndarray con overlays.
    stats: dict con 'estado', 'fase', 't_restante', 'live_counts',
           'rates_vpm', 'tiempos'(acumulados).
    """
    # Cargar config base
    ...
    WEIGHTS = (
        ...
        if weights_path
        else (ROOT / "models" / "yolo12" / "modelo_final" / "weights" / "best.pt")
    )
    ...
    if lanes_config:
        lc_cfg["lanes_config"] = lanes_config
    ...
    if ctrl_overrides:
        ctrl_cfg.update(ctrl_overrides)
    ...
    # Abrir fuente
    src = str(video_source)
    video_path = src
    if src != "0" and not (src.startswith("rtsp://") or src.startswith("rtmp://")):
    ...
    # Writer opcional si save_path existe
    writer = None
    if save_path:
    ...
    try:
        while True:
            ...
            # Emitir frame y métricas
            stats = {
                "estado": estado,
                "fase": active_phase,
                "t_restante": t_restante,
                "live_counts": dict(live_counts),
                "rates_vpm": dict(rates_vpm),
                "tiempos": dict(tiempos),
            }
            yield frame, stats

    finally:
        cap.release()
        if save_path and writer is not None:
            writer.release()
# === FIN NUEVO ==========================================================================
...
def main():
    ...
    # ===== NUEVO: switches desde variables de entorno =====
    HEADLESS = os.getenv("HEADLESS", "0") == "1"
    if os.getenv("VIDEO_SOURCE"):
        app["video_source"] = os.getenv("VIDEO_SOURCE")
    if os.getenv("LANES_CONFIG"):  # permitir cambiar el lanes.json
        app.setdefault("line_counting", {})
        app["line_counting"]["lanes_config"] = os.getenv("LANES_CONFIG")
    # ======================================================

    # Ruta de pesos: permite override por WEIGHTS_PATH
    WEIGHTS = ROOT / "models" / "yolo12" / "modelo_final" / "weights" / "best.pt"
    # ===== NUEVO: override de pesos por entorno =====
    if os.getenv("WEIGHTS_PATH"):
        WEIGHTS = Path(os.getenv("WEIGHTS_PATH")).resolve()
    # =================================================
    ...
    writer = None
    # ===== NUEVO: salida configurable OUT_DIR y OUT_FILE =====
    out_dir = Path(os.getenv("OUT_DIR", "/outputs"))
    out_file_env = os.getenv("OUT_FILE")  # permite nombrar el archivo
    if HEADLESS:
        # si /outputs no existe (p.ej. fuera de contenedor), usa ./outputs local
        if not out_dir.exists():
            out_dir = Path("outputs")
        out_dir.mkdir(parents=True, exist_ok=True)
        out_file = out_dir / (out_file_env if out_file_env else "out.mp4")
    # ========================================================
    ...
    try:
        ...
        # ===== NUEVO: resumen por consola =====
        dur = time.time() - t0_sesion
        print(f"[RESUMEN] sesión={dur:.1f}s  verde={tiempos['verde']:.1f}s  "
              f"amarillo={tiempos['amarillo']:.1f}s  rojo={tiempos['rojo']:.1f}s")
